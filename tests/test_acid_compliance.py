...
def create_test_db_with_fields(
    db_path: Path, indexed_fields: list[str] | None = None
) -> KenobiX:
    """Create a test database with specified indexed fields."""
    return KenobiX(str(db_path), indexed_fields=indexed_fields)
...
class TestAtomicity:
    ...
    def test_atomicity_multiple_inserts(self, db):
    ...
    def test_atomicity_mixed_operations(self, db_path):
        """Mixed insert/update/delete operations are atomic."""
        db = create_test_db_with_fields(db_path, indexed_fields=["name"])
    ...
    def test_atomicity_nested_transactions(self, db):
    ...
    def test_atomicity_large_batch(self, db):
    ...
    def test_atomicity_with_savepoints(self, db):
    ...
    def test_atomicity_odm_operations(self, db):
...
class TestConsistency:
    ...
    def test_consistency_balance_transfer(self, db_path):
        """Balance transfers maintain total balance invariant."""
        db = create_test_db_with_fields(db_path, indexed_fields=["account_id"])
    ...
    def test_consistency_no_negative_balance(self, db_path):
    ...
    def test_consistency_referential_integrity(self, db_path):
        """Related records remain consistent."""
        db = create_test_db_with_fields(
            db_path, indexed_fields=["user_id", "order_id", "type"]
        )
    ...
    def test_consistency_counter_increment(self, db_path):
        """Counter increments maintain correct sequence."""
        db = create_test_db_with_fields(db_path, indexed_fields=["counter_id"])
    ...
    def test_consistency_inventory_tracking(self, db_path):
        """Inventory counts remain accurate."""
        db = create_test_db_with_fields(db_path, indexed_fields=["product_id"])
...
class TestIsolation:
    ...
    def test_isolation_read_committed(self, db_path):
    ...
    def test_isolation_concurrent_transactions(self, db_path):
        ...
        with multiprocessing.Pool(processes=num_workers) as pool:
            results = pool.starmap(
                ...
                [(str(db_path), i, iterations) for i in range(num_workers)],
            )
    ...
    def test_isolation_readers_during_write_transaction(self, db_path):
    ...
    def test_isolation_serializable_transactions(self, db_path):
        ...
        db = KenobiX(str(db_path), indexed_fields=["account_id"])
        account_a = db.search("account_id", "A")[0]
        account_b = db.search("account_id", "B")[0]
...
class TestDurability:
    ...
    def test_durability_simple_commit(self, db_path):
    ...
    def test_durability_multiple_transactions(self, db_path):
    ...
    def test_durability_with_rollback(self, db_path):
    ...
    def test_durability_wal_mode(self, db_path):
    ...
    def test_durability_large_transaction(self, db_path):
    ...
    def test_durability_crash_recovery_simulation(self, db_path):
    ...
    def test_durability_odm_persistence(self, db_path):
...
class TestCombinedACID:
    ...
    def test_acid_banking_scenario(self, db_path):
        ...
        for i in range(50):
            ...
            with db.transaction():
                ...
                if source_account["balance"] >= amount:
                    ...
                    db.update("account_id", dest, {"balance": dest_account["balance"]})
        ...
        final_total = sum(
            db.search("account_id", acc)[0]["balance"] for acc in ["A", "B", "C", "D"]
        )
    ...
    def test_acid_ecommerce_scenario(self, db_path):
        ...
        db = create_test_db_with_fields(db_path, indexed_fields=["order_id", "status"])
        Document.set_database(db)
        ...
        # Verify durability
        db.close()
        db = KenobiX(str(db_path), indexed_fields=["order_id", "status"])
        Document.set_database(db)
