...
class TestConcurrency:
    ...
    def test_multiple_concurrent_readers(self):
        ...
        try:
            ...
            db = KenobiX(db_path, indexed_fields=["worker_id"])
            for i in range(100):
            ...
            with multiprocessing.Pool(processes=num_workers) as pool:
                ...
                results = pool.starmap(
                    ...
                    [(db_path, i, iterations_per_worker) for i in range(num_workers)],
                )
                elapsed = time.time() - start
        ...
        try:
            ...
            with multiprocessing.Pool(processes=num_workers) as pool:
                ...
                results = pool.starmap(
                    ...
                    [
                        (db_path, i, iterations_per_worker, True)
    ...
    def test_concurrent_readers_and_writers(self):
        ...
        try:
            ...
            # Add writer tasks
            tasks.extend(
                (db_path, i, iterations, indexed_fields) for i in range(num_writers)
            )
            ...
            print(
                f"  Combined rate: {(total_reads + total_writes) / elapsed:.0f} ops/sec"
            )
    ...
    def test_database_integrity_after_concurrent_access(self):
        ...
        try:
            ...
            with multiprocessing.Pool(processes=num_workers) as pool:
                pool.starmap(
                    concurrent_writer_worker,
                    [(db_path, i, iterations, True) for i in range(num_workers)],
                )

            # Verify database integrity
            db = KenobiX(db_path, indexed_fields=["worker_id", "iteration"])
